ig.module("game.entities.result")
	...
	.defines(function() {
		EntityResult = ig.Entity.extend({
			...
			sprite: new ig.Image("media/graphics/bg-result.png"),
					...
					if (data.hasOwnProperty(key)) {
						...
						this.lists.push(
							ig.game.spawnEntity(
								EntityList,
								this.pos.x,
								this.pos.y + this.list_height + this.list_height * key,
								{
									index: Number(key) + 1,
									name: element[0],
									reward: element[1],
									...
									rect: {
										x: this.pos.x + 5,
										y: this.pos.y + this.list_height - 5,
										w: this.sprite.width - 10,
										h: this.sprite.height - this.list_height
									}
								}
							)
						);
			...
			draw: function() {
				...
				ctx.fillText("รอบที่" + (this.round == "round1" ? "1" : "2"), this.pos.x + 15, this.pos.y - 20);
				ctx.fillText("ลำดับ", this.pos.x + 15, this.pos.y + 50);
				ctx.fillText("รายชื่อ", this.pos.x + 79, this.pos.y + 50);
				ctx.fillText("ได้รางวัล", this.pos.x + 350, this.pos.y + 50);
				ctx.fillText("เวลา", this.pos.x + 455, this.pos.y + 50);
			}
		});
		EntityList = ig.Entity.extend({
			...
			draw: function() {
				...
				ctx.fillRect(this.pos.x, this.pos.y, 536, 88);

				ctx.font = "24px DBHeaventRounded";
				ctx.textAlign = "left";
				ctx.textBaseline = "alphabetic";
				ctx.fillStyle = "#2F353A";
				ctx.fillText(this.index, this.pos.x + 15, this.pos.y + 50);
				ctx.fillText(this.name, this.pos.x + 79, this.pos.y + 50);
				ctx.fillText(this.reward, this.pos.x + 350, this.pos.y + 50);
				ctx.fillText(this.time, this.pos.x + 455, this.pos.y + 50);
